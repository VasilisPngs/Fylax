...
public sealed class MainWindow : Window
{
    ...
    private SolidColorBrush? _toggleThumbBrush;
    private SolidColorBrush? _toggleBorderBrush;
    ...
    public MainWindow(bool autoStartProtection = false)
    {
        ...
        _theme = ResolveTheme();
        EnsureFluentScrollbarStyle();
        UpdateFluentScrollBrushes();
        ...
        {
            System.Windows.Application.Current.SessionEnding += (_, _) => Cleanup();
        }
    ...
    {
        _startupService.RemoveLegacyRunKey();
    ...
    private const string FluentScrollbarXaml = """
<ResourceDictionary xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation" xmlns:x="http://schemas.microsoft.com/winfx/2006/xaml">
  <Style x:Key="FluentScrollPage" TargetType="RepeatButton">
  ...
  <Style x:Key="FluentScrollArrow" TargetType="RepeatButton">
    <Setter Property="Focusable" Value="False"/>
    <Setter Property="Template">
      <Setter.Value>
        <ControlTemplate TargetType="RepeatButton">
          <Border Background="{DynamicResource FluentScrollTrackBrush}">
            <TextBlock x:Name="ArrowGlyph" Text="{TemplateBinding Content}" FontFamily="Segoe Fluent Icons" FontSize="9" Foreground="{DynamicResource FluentScrollArrowBrush}" HorizontalAlignment="Center" VerticalAlignment="Center"/>
          </Border>
          <ControlTemplate.Triggers>
            <Trigger Property="IsMouseOver" Value="True">
              <Setter TargetName="ArrowGlyph" Property="Foreground" Value="{DynamicResource FluentScrollArrowActiveBrush}"/>
            </Trigger>
  ...
  <ControlTemplate x:Key="FluentScrollBarVertical" TargetType="ScrollBar">
    <Grid Background="{DynamicResource FluentScrollTrackBrush}">
      <Grid.RowDefinitions>
        <RowDefinition Height="18"/>
        <RowDefinition Height="*"/>
        <RowDefinition Height="18"/>
      </Grid.RowDefinitions>
      <RepeatButton Grid.Row="0" Style="{StaticResource FluentScrollArrow}" Command="{x:Static ScrollBar.LineUpCommand}" Content="&#xEDDB;"/>
      <Track x:Name="PART_Track" Grid.Row="1" IsDirectionReversed="True">
        <Track.DecreaseRepeatButton>
          <RepeatButton Style="{StaticResource FluentScrollPage}" Command="{x:Static ScrollBar.PageUpCommand}"/>
        </Track.DecreaseRepeatButton>
        <Track.IncreaseRepeatButton>
          <RepeatButton Style="{StaticResource FluentScrollPage}" Command="{x:Static ScrollBar.PageDownCommand}"/>
        </Track.IncreaseRepeatButton>
        <Track.Thumb>
          <Thumb Focusable="False">
            <Thumb.Template>
              <ControlTemplate TargetType="Thumb">
                <Border x:Name="ThumbBar" Width="9" CornerRadius="4.5" Background="{DynamicResource FluentScrollThumbBrush}" HorizontalAlignment="Center"/>
                <ControlTemplate.Triggers>
                  <Trigger Property="IsMouseOver" Value="True">
                    <Setter TargetName="ThumbBar" Property="Background" Value="{DynamicResource FluentScrollThumbActiveBrush}"/>
                  </Trigger>
      ...
      <RepeatButton Grid.Row="2" Style="{StaticResource FluentScrollArrow}" Command="{x:Static ScrollBar.LineDownCommand}" Content="&#xEDDC;"/>
    </Grid>
  </ControlTemplate>
  <ControlTemplate x:Key="FluentScrollBarHorizontal" TargetType="ScrollBar">
    <Grid Background="{DynamicResource FluentScrollTrackBrush}">
      ...
      <RepeatButton Grid.Column="0" Style="{StaticResource FluentScrollArrow}" Command="{x:Static ScrollBar.LineLeftCommand}" Content="&#xEDD9;"/>
      <Track x:Name="PART_Track" Grid.Column="1">
        <Track.DecreaseRepeatButton>
          <RepeatButton Style="{StaticResource FluentScrollPage}" Command="{x:Static ScrollBar.PageLeftCommand}"/>
        </Track.DecreaseRepeatButton>
        <Track.IncreaseRepeatButton>
          <RepeatButton Style="{StaticResource FluentScrollPage}" Command="{x:Static ScrollBar.PageRightCommand}"/>
        </Track.IncreaseRepeatButton>
        <Track.Thumb>
          <Thumb Focusable="False">
            <Thumb.Template>
              <ControlTemplate TargetType="Thumb">
                <Border x:Name="ThumbBar" Height="9" CornerRadius="4.5" Background="{DynamicResource FluentScrollThumbBrush}" VerticalAlignment="Center"/>
                <ControlTemplate.Triggers>
                  <Trigger Property="IsMouseOver" Value="True">
                    <Setter TargetName="ThumbBar" Property="Background" Value="{DynamicResource FluentScrollThumbActiveBrush}"/>
                  </Trigger>
      ...
      <RepeatButton Grid.Column="2" Style="{StaticResource FluentScrollArrow}" Command="{x:Static ScrollBar.LineRightCommand}" Content="&#xEDDA;"/>
    </Grid>
  </ControlTemplate>
  <Style TargetType="ScrollBar">
    <Setter Property="Width" Value="15"/>
    <Setter Property="MinWidth" Value="15"/>
    <Setter Property="Template" Value="{StaticResource FluentScrollBarVertical}"/>
    <Style.Triggers>
      <Trigger Property="Orientation" Value="Horizontal">
        ...
        <Setter Property="Template" Value="{StaticResource FluentScrollBarHorizontal}"/>
      </Trigger>
    </Style.Triggers>
  </Style>
</ResourceDictionary>
""";
    ...
    private static bool _fluentScrollbarStyleApplied;

    private static void EnsureFluentScrollbarStyle()
    {
        if (_fluentScrollbarStyleApplied)
        {
            return;
        }
        ...
        try
        {
            var dictionary = (ResourceDictionary)System.Windows.Markup.XamlReader.Parse(FluentScrollbarXaml);
            app.Resources.MergedDictionaries.Add(dictionary);
            _fluentScrollbarStyleApplied = true;
        }
        catch
        {
        }
    ...
    private void UpdateFluentScrollBrushes()
    {
        ...
        resources["FluentScrollTrackBrush"] = new SolidColorBrush(dark ? Color.FromRgb(0x2C, 0x2C, 0x2C) : Color.FromRgb(0xFC, 0xFC, 0xFC));
        resources["FluentScrollThumbBrush"] = new SolidColorBrush(dark ? Color.FromRgb(0x9F, 0x9F, 0x9F) : Color.FromRgb(0x8B, 0x8B, 0x8B));
        resources["FluentScrollThumbActiveBrush"] = new SolidColorBrush(dark ? Color.FromRgb(0xD1, 0xD1, 0xD1) : Color.FromRgb(0x63, 0x63, 0x63));
        resources["FluentScrollArrowBrush"] = new SolidColorBrush(dark ? Color.FromRgb(0x9F, 0x9F, 0x9F) : Color.FromRgb(0x8B, 0x8B, 0x8B));
        resources["FluentScrollArrowActiveBrush"] = new SolidColorBrush(dark ? Color.FromRgb(0xD1, 0xD1, 0xD1) : Color.FromRgb(0x63, 0x63, 0x63));
    }
    ...
    private void SetupTray()
    {
        try
        {
            ...
            try
            {
                var info = System.Windows.Application.GetResourceStream(new Uri("pack://application:,,,/tray.ico"));
    ...
    {
        var bar = new UniformGrid { Rows = 1, Columns = 4 };
        bar.Children.Add(NavItem(Icons.Home, 0));
        bar.Children.Add(NavItem(Icons.SearchActivity, 1));
        bar.Children.Add(NavItem(Icons.Shield, 2));
        bar.Children.Add(NavItem(Icons.Settings, 3));
    ...
    private FrameworkElement NavItem(string path, int index)
    {
        ...
        cell.MouseLeftButtonUp += (_, _) => SetTab(index);
        return cell;
    }
    ...
    private FrameworkElement BuildHome()
    {
        ...
        _homeSubtitle = Label(_active && (_settings.EnabledUrls().Count > 0 || _settings.BlockedManual.Count > 0) ? "DNS filtering is enabled" : "DNS filtering is disabled", 15, _theme.OnSurfaceVariant);
        ...
        var container = new Grid { Margin = new Thickness(24, 16, 24, 24) };
        container.Children.Add(column);

        var appTitle = Label("Fylax", 22, _theme.OnSurface);
        appTitle.HorizontalAlignment = HorizontalAlignment.Center;
        appTitle.VerticalAlignment = VerticalAlignment.Top;
        ...
        refresh.Margin = new Thickness(0, -8, -16, 0);
    ...
    private FrameworkElement BuildHomeToggle()
    {
        ...
        _toggleTrackBrush = new SolidColorBrush(_active ? ((SolidColorBrush)_theme.Primary).Color : ((SolidColorBrush)_theme.SurfaceContainerHighest).Color);
        _toggleBorderBrush = new SolidColorBrush(_active ? ((SolidColorBrush)_theme.Primary).Color : ((SolidColorBrush)_theme.Outline).Color);
        _toggleThumbBrush = new SolidColorBrush(_active ? ((SolidColorBrush)_theme.OnPrimary).Color : ((SolidColorBrush)_theme.Outline).Color);
        ...
        var track = new Border
        {
            ...
            BorderBrush = _toggleBorderBrush,
            BorderThickness = new Thickness(4)
        };

        var thumbShape = new Ellipse
        {
            Width = _active ? 53 : 35,
            Height = _active ? 53 : 35,
            Fill = _toggleThumbBrush
        };

        _toggleThumb = thumbShape;

        var canvas = new Canvas { Width = w, Height = h };
        Canvas.SetTop(thumbShape, _active ? 8 : 18);
        Canvas.SetLeft(thumbShape, _active ? 53 : 18);
    ...
    private void SetActiveVisual(bool on)
    {
        ...
        _toggleThumb?.BeginAnimation(Canvas.LeftProperty, new DoubleAnimation(on ? 53 : 18, duration) { EasingFunction = ease });
        _toggleThumb?.BeginAnimation(Canvas.TopProperty, new DoubleAnimation(on ? 8 : 18, duration) { EasingFunction = ease });
        _toggleThumb?.BeginAnimation(FrameworkElement.WidthProperty, new DoubleAnimation(on ? 53 : 35, duration) { EasingFunction = ease });
        _toggleThumb?.BeginAnimation(FrameworkElement.HeightProperty, new DoubleAnimation(on ? 53 : 35, duration) { EasingFunction = ease });
        _toggleTrackBrush?.BeginAnimation(SolidColorBrush.ColorProperty, new ColorAnimation(on ? ((SolidColorBrush)_theme.Primary).Color : ((SolidColorBrush)_theme.SurfaceContainerHighest).Color, duration));
        _toggleBorderBrush?.BeginAnimation(SolidColorBrush.ColorProperty, new ColorAnimation(on ? ((SolidColorBrush)_theme.Primary).Color : ((SolidColorBrush)_theme.Outline).Color, duration));
        _toggleThumbBrush?.BeginAnimation(SolidColorBrush.ColorProperty, new ColorAnimation(on ? ((SolidColorBrush)_theme.OnPrimary).Color : ((SolidColorBrush)_theme.Outline).Color, duration));
        ...
        {
            _homeSubtitle.Text = on && (_settings.EnabledUrls().Count > 0 || _settings.BlockedManual.Count > 0) ? "DNS filtering is enabled" : "DNS filtering is disabled";
        }
    }
    ...
    private async Task RestartProtectionAsync()
    {
        if (!_dnsProxyService.IsRunning || _busy)
        {
            return;
        }
    ...
    private static string? KnownName(string url)
    {
        var u = url.ToLowerInvariant();

        if (u.Contains("hagezi") && u.Contains("pro.plus")) return "HaGeZi Pro Plus";
        if (u.Contains("hagezi") && u.Contains("ultimate")) return "HaGeZi Ultimate";
        if (u.Contains("hagezi") && u.Contains("multi")) return "HaGeZi Multi";
        if (u.Contains("hagezi") && u.Contains("light")) return "HaGeZi Light";
        if (u.Contains("hagezi") && u.Contains("pro")) return "HaGeZi Pro";
        if (u.Contains("hagezi") && u.Contains("normal")) return "HaGeZi Normal";
        if (u.Contains("hagezi")) return "HaGeZi";
        if (u.Contains("adguardsdnsfilter") || u.Contains("adguard-dns-filter")) return "AdGuard DNS filter";
        if (u.Contains("adguardteam")) return "AdGuard filter";
        if (u.Contains("oisd") && u.Contains("small")) return "OISD Small";
        if (u.Contains("oisd")) return "OISD Big";
        if (u.Contains("stevenblack")) return "StevenBlack Hosts";
        if (u.Contains("easyprivacy")) return "EasyPrivacy";
        if (u.Contains("easylist")) return "EasyList";
        if (u.Contains("pgl.yoyo") || u.Contains("peter-lowe")) return "Peter Lowe's List";
        if (u.Contains("someonewhocares")) return "Dan Pollock's List";
        if (u.Contains("1hosts")) return "1Hosts";
        if (u.Contains("notracking")) return "NoTracking";
        if (u.Contains("dandelionsprout")) return "DandelionSprout";
        if (u.Contains("phishing.army")) return "Phishing Army";
        if (u.Contains("urlhaus")) return "URLhaus";
        if (u.Contains("firebog")) return "The Firebog";

        return null;
    }

    private static string DisplayName(BlockListItem item)
    {
        ...
        return Uri.TryCreate(item.Url, UriKind.Absolute, out var parsed) ? parsed.Host : item.Url;
    }
    ...
    private string? PromptForName()
    {
        ...
        panel.Children.Add(Label("Name this list", 16, _theme.OnSurface));
        panel.Children.Add(Spacer(12));

        var field = MakeField("Name", "", out var box);
        panel.Children.Add(field);
        panel.Children.Add(Spacer(16));
        ...
        buttons.Children.Add(TextOnlyButton("Cancel", () => dialog.Close()));
        buttons.Children.Add(TextOnlyButton("Add", () =>
        {
    ...
    {
        var resolvers = new List<IUpstreamResolver>();

        AddCustomResolver(resolvers, _settings.DnsPrimary);
        AddCustomResolver(resolvers, _settings.DnsSecondary);
        ...
        return resolvers.Count == 1 ? resolvers[0] : new FallbackUpstreamResolver(resolvers);
    }

    private void AddCustomResolver(List<IUpstreamResolver> resolvers, string value)
    {
        ...
        if (_settings.DohEnabled)
        {
            ...
            resolvers.Add(new DohUpstreamResolver($"https://{host}/dns-query"));
        }
        else
        {
            resolvers.Add(new PlainUpstreamResolver(ip));
        }
    }
    ...
    private FrameworkElement BuildActivity()
    {
        ...
        var activityTitle = Label("Activity", 22, _theme.OnSurface);
        activityTitle.HorizontalAlignment = HorizontalAlignment.Center;
        top.Children.Add(activityTitle);
        ...
        reqColumn.Children.Add(Label("Requests", 14, _theme.OnSurfaceVariant));
        Grid.SetColumn(reqColumn, 0);

        var blkColumn = new StackPanel();
        _blkText = Label(counts.Blocked.ToString(), 26, _theme.Error);
        blkColumn.Children.Add(_blkText);
        blkColumn.Children.Add(Label("Blocked", 14, _theme.OnSurfaceVariant));
    ...
    private void RefreshActivityList()
    {
        ...
        if (filtered.Count == 0)
        {
            var empty = Label("No activity yet", 14, _theme.OnSurfaceVariant);
    ...
    private FrameworkElement ActivityRow(DnsQueryLogEntry entry)
    {
        ...
        var host = new TextBlock
        {
            ...
            FontSize = 14,
            FontFamily = AppFont,
            Foreground = entry.Blocked ? _theme.Error : _theme.OnSurface,
    ...
    private FrameworkElement BuildShield()
    {
        ...
        var title = Label("Protection", 22, _theme.OnSurface);
        title.HorizontalAlignment = HorizontalAlignment.Center;
        content.Children.Add(title);
        content.Children.Add(Spacer(24));

        content.Children.Add(Label("DNS Filters", 22, _theme.OnSurface));
        ...
        var addUrlButton = IconButton(Icons.Add, () =>
        {
            ...
            var name = KnownName(url) ?? PromptForName();
            ...
            _settings.Lists.Add(new BlockListItem { Url = url, Enabled = true, Name = name });
            _settingsService.Save(_settings);
            _expandedList = -1;
            RebuildLayout();
            _ = RestartProtectionAsync();
        ...
        content.Children.Add(Label("Allowed", 22, _theme.OnSurface));
        content.Children.Add(Spacer(12));
        ...
        content.Children.Add(Label("Blocked", 22, _theme.OnSurface));
        content.Children.Add(Spacer(12));
        ...
        content.Children.Add(Label("DNS Server", 22, _theme.OnSurface));
        ...
        if (_settings.DnsMode == DnsMode.Custom)
        {
            ...
            var dohLabel = Label("DNS over HTTPS (DoH)", 16, _theme.OnSurface);
        ...
        content.Children.Add(Spacer(12));
        content.Children.Add(PillButton("Apply", async () =>
        {
            await SaveDnsAsync();
            await RestartProtectionAsync();
        }));
    ...
    {
        var index = _settings.Lists.IndexOf(item);
        ...
        {
            _expandedList = _expandedList == index ? -1 : index;
            RebuildLayout();
        };
        Grid.SetColumn(name, 0);

        var toggle = MakeSwitch(item.Enabled, value =>
        {
            item.Enabled = value;
            _settingsService.Save(_settings);
            _ = RestartProtectionAsync();
        ...
        if (_expandedList == index)
        {
            ...
            urlText.MouseLeftButtonUp += (_, _) => OpenInBrowser(item.Url);
    ...
    {
        var top = new StackPanel();
        var settingsTitle = Label("Settings", 22, _theme.OnSurface);
        settingsTitle.HorizontalAlignment = HorizontalAlignment.Center;
        top.Children.Add(settingsTitle);
        top.Children.Add(Spacer(24));
        top.Children.Add(Label("Theme", 22, _theme.OnSurface));
        top.Children.Add(Spacer(8));
        ...
        top.Children.Add(Spacer(24));
        top.Children.Add(Label("Startup", 22, _theme.OnSurface));
        top.Children.Add(Spacer(8));
        ...
        var startupLabel = Label("Start with Windows", 16, _theme.OnSurface);
        ...
        var name = Label("Fylax", 22, _theme.OnSurface);
    ...
    private void ShowSnack(string message)
    {
        ...
        var snack = new Border
        {
            ...
            HorizontalAlignment = HorizontalAlignment.Center,
            VerticalAlignment = VerticalAlignment.Bottom,
            Margin = new Thickness(24, 0, 24, 76),
    ...
    private FrameworkElement MakeSwitch(bool initial, Action<bool> onChange)
    {
        ...
        var onTrack = ((SolidColorBrush)_theme.Primary).Color;
        var offTrack = ((SolidColorBrush)_theme.SurfaceContainerHighest).Color;
        var onThumb = ((SolidColorBrush)_theme.OnPrimary).Color;
        var offThumb = ((SolidColorBrush)_theme.Outline).Color;

        var state = initial;
        var trackBrush = new SolidColorBrush(state ? onTrack : offTrack);
        var borderBrush = new SolidColorBrush(state ? onTrack : offThumb);
        var thumbBrush = new SolidColorBrush(state ? onThumb : offThumb);
        ...
        var canvas = new Canvas { Width = w, Height = h };
        Canvas.SetTop(thumbShape, state ? 4 : 8);
        Canvas.SetLeft(thumbShape, state ? 24 : 8);
        canvas.Children.Add(track);
        canvas.Children.Add(thumbShape);
        ...
        host.MouseLeftButtonUp += (_, _) =>
        {
            ...
            thumbShape.BeginAnimation(Canvas.LeftProperty, new DoubleAnimation(state ? 24 : 8, duration) { EasingFunction = ease });
            thumbShape.BeginAnimation(Canvas.TopProperty, new DoubleAnimation(state ? 4 : 8, duration) { EasingFunction = ease });
            thumbShape.BeginAnimation(FrameworkElement.WidthProperty, new DoubleAnimation(state ? 24 : 16, duration) { EasingFunction = ease });
            thumbShape.BeginAnimation(FrameworkElement.HeightProperty, new DoubleAnimation(state ? 24 : 16, duration) { EasingFunction = ease });
            trackBrush.BeginAnimation(SolidColorBrush.ColorProperty, new ColorAnimation(state ? onTrack : offTrack, duration));
            borderBrush.BeginAnimation(SolidColorBrush.ColorProperty, new ColorAnimation(state ? onTrack : offThumb, duration));
            thumbBrush.BeginAnimation(SolidColorBrush.ColorProperty, new ColorAnimation(state ? onThumb : offThumb, duration));
            onChange(state);
        };
